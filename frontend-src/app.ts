...
export class A2UILayoutEditor extends SignalWatcher(LitElement) {
  ...
  render() {
    return html`
      ${this.#renderThemeToggle()} ${this.#renderJsonToggle()}
      <div class=${this._showJson ? 'workspace split' : 'workspace'}>
        <main class="preview-pane">
          ${this.#renderLocalModeHeader()} ${this.#maybeRenderForm()} ${this.#maybeRenderData()}
          ${this.#maybeRenderError()}
        </main>
        ${this._showJson ? this.#renderJsonInspector() : nothing}
      </div>
      ${this.#renderToast()}
    ...
    return html`
      <label class="json-toggle">
        <input
          ...
          @change=${(evt: Event) => {
            this._showJson = (evt.target as HTMLInputElement).checked;
          }}
        />
      ...
      <aside class="json-inspector" aria-label="A2UI JSON segments">
        ...
        ${
          this._lastMessages.length === 0
            ? html`<p class="json-empty">
                Send a request to see the A2UI JSON used to render the current view.
              </p>`
            : this._lastMessages.map(
                (message, index) => html`
                  <section class="json-segment">
                    <div class="json-segment-title">Segment ${index + 1}</div>
                    <pre><code>${this.#formatJson(message)}</code></pre>
                  </section>
                `,
              )
        }
